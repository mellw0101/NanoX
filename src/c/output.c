...
static buf_break_st_t *buf_break_list = NULL;
buf_break_st_t        *buf_break      = NULL;

int out_lines = 0; /* <-- Used 'here' and 'indent.c'. */
int com_lines = 0; /* <-- Used 'here', 'indent.c' and 'comments.c'. */
 ...
 */
static bool
better_break(buf_break_st_t *b1, const buf_break_st_t *b2)
{
 ...
 */
static void
set_priority(buf_break_st_t *bb)
{
...
void
clear_buf_break_list(bool *pbreak_line)
{
    buf_break_st_t *bb;
...
static void
set_next_buf_break(int prev_code_target, int new_code_target, int offset, bool *pbreak_line)
{
    buf_break_st_t *bb;
 ...
 */
static int
pad_output(int cur_col, int target_column)
{
    int offset       = 0;
        ...
        if (settings.use_tabs && (settings.tabsize > 1))
        {
            if (settings.align_with_spaces)
            {
                    ...
                    tos > 0)
                {
                    do
                    {
                        if (parser_state_tos->p_stack[tos] == ifstmt || parser_state_tos->p_stack[tos] == forstmt ||
                ...
                else if (parser_state_tos->p_stack[tos] == ifstmt || parser_state_tos->p_stack[tos] == forstmt ||
                         parser_state_tos->p_stack[tos] == whilestmt)
                {
                    align_target = parser_state_tos->il[tos];
                }
...
static void
output_substring(FILE *file, const char *begin, const char *end)
{
    const char *p;
...
dump_line_label(void)
{
    ...
    if ((settings.force_preproc_width > 0) && (s_lab[0] == '#'))
    {
        ...
        else if ((strncmp(p, "if", 2) == 0) || (strncmp(p, "ifdef", 5) == 0))
        {}
    ...
    else if ((s_lab[0] == '#') && ((strncmp(&s_lab[1], "else", 4) == 0) || (strncmp(&s_lab[1], "endif", 5) == 0)))
    {
...
static int
count_parens(const char *string)
{
    int paren_level = 0;
...
dump_line_code(int *pcur_col, int *pnot_truncated, int paren_targ, bool *pbreak_line, int target_col_break)
{
    ...
    if (s_code != e_code)
    {
        ...
        /**
         * If a line ends in an lparen character, the following line should not
         * line up with the parenthesis, but should be indented by the usual amount.
         */
...
dump_line(int force_nl, int *paren_targ, bool *pbreak_line)
{
    ...
    {
        /* If we have a formfeed on a blank line, we should just
         * output it, rather than treat it as a normal blank line.  */
    ...
    else
    {
        ...
        {
            if ((prefix_blankline_requested_code != decl) || !parser_state_tos->decl_on_line)
            {
                n_real_blanklines = 1;
            }
        }
    ...
    *s_com = '\0';
    ...
    if (inhibited)
    {
        ...
        do
        {
            ...
            if ((*p == '/') && ((*(p + 1) == '*') || (*(p + 1) == '/')))
            {
                /* We've hit a comment.  See if turns formatting back on. */
                putc(*p++, output);
                putc(*p++, output);
                while ((*p == ' ') || (*p == TAB))
                {
...
/* Flushes any buffered output to the output file. */
extern void
flush_output(void)
{
    fflush(output);
...
open_output(const char *filename, const char *mode)
{
    ...
    {
        output = fopen(filename, mode); /* open the file for read + write (but see the trunc function) */
        if (output == NULL)
        {
            fatal("indent: can't create %s\n", filename);
        }
    }
}

...
int monitor_count(void) {
  int ret;
  SDL_DisplayID *mons;
  ALWAYS_ASSERT(mons = SDL_GetDisplays(&ret));
  SDL_free(mons);
  return ret;
}
...
int monitor_return_first_monitor_rate(void) {
  int count;
  return ROUNDF(monitor_get_mode(monitor_get_all(&count)[0])->refresh_rate);
}
...
SDL_DisplayID *monitor_get_all(int *const count) {
  SDL_DisplayID *mons;
  ALWAYS_ASSERT(mons = SDL_GetDisplays(count));
  return mons;
}

/* ----------------------------- Monitor get mode ----------------------------- */

const SDL_DisplayMode *monitor_get_mode(SDL_DisplayID id) {
  const SDL_DisplayMode *mode;
  ALWAYS_ASSERT(mode = SDL_GetCurrentDisplayMode((id)));
  return mode;
}
...
int *monitor_refresh_rate_array(int *const count) {
  SDL_DisplayID *mons = monitor_get_all(count);
  int *ret = xmalloc(sizeof(int) * (*count));
  for (int i=0; i<(*count); ++i) {
    ret[i] = ROUNDF(monitor_get_mode(mons[i])->refresh_rate);
  }
  SDL_free(mons);
  return ret;
}

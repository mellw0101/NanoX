...
const char *settings_strings[6] = {
    KR_SETTINGS_STRING, GNU_SETTINGS_STRING, ORIG_SETTINGS_STRING, LINUX_SETTINGS_STRING, "-ip0\0", VERSION};
...
typedef struct
{
    /* Option name, e.g. 'bl', 'cli'. */
    const char *p_name;
    /* Profile type. */
    profile_ty p_type;
    /* The default value (if PRO_BOOL or PRO_INT). */
    int p_default;
    /**
     * If p_type == PRO_BOOL, ON or OFF to tell how this switch
     * affects the variable.  Not used for other p_type's.
     */
    on_or_off_ty p_special;
    /**
     * If p_type == PRO_SETTINGS, a (char *) pointing to a list of the switches
     * to set, separated by NULLs, terminated by 2 NULLs.  If p_type == PRO_BOOL
     * or PRO_INT, the address of the variable that gets set by the option.
     * If p_type == PRO_PRSTRING, an index into the settings_strings array.
...
{
    const int firstwidth  = strlen(shortflag);
    const int secondwidth = strlen(longflag);
    printf(" %s", shortflag);
    if (firstwidth < 14)
    {
...
/* Print the usage line for the given option to the screen. */
void
print_short_opt(const char *const shortflag, const char *const description)
{
    const int firstwidth = strlen(shortflag);
    printf(" %s", shortflag);
    if (firstwidth < 14)
...
{
    printf("\n%s\n\n"
           "Usage: %s [FILE] [OUTPUT] [OPTIONS]\n"
           "Options:\n",
           PACKAGE_STRING, PACKAGE);
    print_short_opt("-version", "Print version.");
    print_short_opt("-o", "Print output to file.");
    print_short_opt("-st", "Print output to stdout.");
    print_short_opt("-help", "Print this text.");
 ...
 */
static bool
eqin(const char *s1, const char *s2, const char **start_param)
{
    bool ret = true;
    while (*s1)
    {
...
set_defaults(void)
{
    ...
    {
        if (p->p_obj && (((p->p_type == PRO_BOOL) && (p->p_special == ON)) || (p->p_type == PRO_INT)))
        {
 ...
 */
static void
arg_missing(const char *option, const char *option_source)
{
    DieError(invocation_error, "%s: missing argument to parameter %s\n", option_source, option);
}

/*!< Strings which can prefix an option, longest first. */
static const char *option_prefixes[4] = {"--", "-", "+", NULL};
 ...
 */
extern int
set_option(const char *option, const char *param, int expl, const char *option_source)
{
    const pro_ty *p             = pro;
    ...
    if (!found)
    {
        DieError(invocation_error, "%s: unknown option \"%s\"\n", option_source, option - 1);
    }
    ...
    {
        /* If the parameter has been explicitly specified, we don't want a
         * group of bundled settings to override the explicit setting. */
        if (settings.verbose)
        {
            fprintf(stderr, "option: %s\n", p->p_name);
        }
        if (expl || !*(p->p_explicit))
        {
            if (expl)
            {
                *(p->p_explicit) = 1;
            ...
            switch (p->p_type)
            {
                ...
                case PRO_INT :
                {
                    ...
                    {
                        DieError(invocation_error, "%s: option ``%s'' requires a numeric parameter\n", option_source, option - 1);
                    }
                    break;
                }
                default :
                {
                    DieError(invocation_error, "set_option: internal error: p_type %d\n", (int)p->p_type);
                }
            }
        }
    }
...
/**
 * The first 2 characters of a c++ comment have been read skip the
 * remainder of the line and return the first character from the next line.
 */
static int
skip_cpp_comment(FILE *f)
{
    int i;
    do
    {
 ...
 * remainder of the comment and return the first character after the comment.
 */
static int
skip_c_comment(FILE *f)
{
    int i = getc(f);
    do
    {
        ...
        if (i == EOF)
        {
            WARNING("Profile contains an unterminated comment", NULL, NULL);
            break;
        }
...
/**
 * The starting / of a comment has been read.  Skip over the rest
 * of the comment and return the first character after the comment.
 */
static int
skip_comment(FILE *f)
{
    ...
    switch (i)
    {
        ...
        default :
            WARNING("Profile contains unpalatable characters", NULL, NULL);
    }
    return i;
}

/**
 * Skip a sequence of space and control characters
 * and return the first character after the sequence.
 */
static int
skip_spaces(FILE *f, int first)
{
    int i = first;
    while ((i <= ' ') && (i != EOF))
    {
 ...
 */
static int
read_string(FILE *f, char *buff, int first)
{
    int   i = first;
    char *p = buff;
    while ((i != EOF) && (i > ' ') && (i != '/') && (p < buff + BUFSIZ))
    {
        *(p++) = (char)i;
        i      = getc(f);
    }
 ...
 */
static void
scan_profile(FILE *f, const char *option_source)
{
    char  b0[BUFSIZ];
    char  b1[BUFSIZ];
    char *current = b0;
    ...
    while (i != EOF)
    {
        ...
        else
        {
            ...
            {
                /* The option had a parameter, thus both values have
                 * been consumed.  Reset the 2 buffers to 'empty' */
                current = b0;
            }
            else
            {
                /* Set option consumed one value. Move the other value
                 * to the first buffer and go get a new second value. */
                strcpy(b0, b1);
                current = b1;
            }
        }
...
set_profile(void)
{
    ...
    if (envname != NULL)
    {
        ...
        if (f == NULL)
        {
            fatal("File named by environment variable %s does not exist or is not readable", PROFILE_ENV_NAME);
        }
        else
        {

#include "../include/c_prototypes.h"

/* Expand a buffer to hold more chars, aligned on a 1 KB boundary. */
extern void
need_chars(buf_t *bp, size_t needed)
{
...
static void
copy_id(const codes_t type_code, bool *force_nl, exit_values_t *file_exit_value, const bb_code_t can_break)
{
    char *t_ptr;
    ...
    *e_code                      = '\0';
    ...
    if ((type_code == sp_paren) &&
        ((!settings.space_after_if && (*token == 'i')) || (!settings.space_after_for && (*token == 'f')) ||
         (!settings.space_after_while && (*token == 'w'))))
    {
        parser_state_tos->want_blank = false;
    }
    /* If the token is one of the GNU gettext macro's '_' or 'N_' then we don't want a blank */
    if ((((token_end - token) == 1) && (*token == '_')) ||
        (((token_end - token) == 2) && (*token == 'N') && (token[1] == '_')))
    {
        parser_state_tos->want_blank = false;
    }
    /* If the token is va_dcl, it appears without a
     * semicolon, so we need to pretend that one was there.  */
...
static void
handle_token_form_feed(bool *pbreak_line)
{
 ...
 *
 * Wow, i was 1 years old when this was written.
 */
static void
handle_token_newline(bool *force_nl, bool *pbreak_line)
{
    ...
    {
        if (((parser_state_tos->last_token != comma) || !settings.leave_comma || !break_comma ||
             (parser_state_tos->p_l_follow > 0) || parser_state_tos->block_init || (s_com != e_com)) &&
            (((parser_state_tos->last_token != rbrace) ||
              !(settings.braces_on_struct_decl_line && parser_state_tos->in_decl))))
        {
            /**
             ...
             */
            if (!settings.procnames_start_line && (s_lab == e_lab) && (parser_state_tos->last_token != lparen) &&
    ...
    /* If we were on the line with a #else or a #endif, we aren't anymore. */
    else_or_endif = false;
...
static void
handle_token_lparen(bool *force_nl, bool *sp_sw, int *dec_ind, bool *pbreak_line)
{
    /**
     ...
     */
    if ((*token == '{') && ((s_code != e_code) || (s_com != e_com) || (s_lab != e_lab)))
    {
        dump_line(true, &paren_target, pbreak_line);
    ...
    if (parser_state_tos->want_blank && (*token != '[') &&
        ((parser_state_tos->last_token != ident) || settings.proc_calls_space ||
    ...
    if (*(token + 1) == '*' && parser_state_tos->last_rw == rw_decl &&
        (parser_state_tos->last_token == decl || parser_state_tos->last_token == unary_op))
    {
    ...
    if (*sp_sw && (parser_state_tos->p_l_follow == 1) && settings.extra_expression_indent &&
    ...
    if ((parser_state_tos->in_or_st == 1) && *token == '(')
    {
         ...
         * correctly aligned if proc decl has an explicit type on it,
         * i.e. "int a(x) {..."
         */
    ...
    if ((*token == '(') && settings.break_function_decl_args && parser_state_tos->in_stmt &&
        parser_state_tos->in_decl && (parser_state_tos->paren_depth == 1))
    {
        dump_line(true, &paren_target, pbreak_line);
        *force_nl    = false;
...
static void
handle_token_rparen(bool *force_nl, bool *sp_sw, codes_t *hd_type, bool *last_token_ends_sp,
                    exit_values_t *file_exit_value, bool *pbreak_line)
{
    char tmpchar[2], *tmp;
    parser_state_tos->paren_depth--;
    /* For declarations, if user wants close of fn decls broken, force that now. */
    if ((*token == ')') && settings.break_function_decl_args_end && !parser_state_tos->in_or_st &&
        parser_state_tos->in_stmt && parser_state_tos->in_decl && (parser_state_tos->paren_depth == 0))
    {
    ...
    *(e_code++) = token[0];
     ...
     */
    if (settings.allow_single_line_conditionals && *(token - 1) == ')' && *(token + 2) != '{' &&
        !parser_state_tos->paren_depth)
    {
        parser_state_tos->want_blank = true;
    }
...
static void
handle_token_unary_op(int *dec_ind, const bb_code_t can_break)
{
    char *t_ptr;
    if (parser_state_tos->want_blank && !(parser_state_tos->in_decl && !settings.pointer_align_right && *token == '*'))
    {
    ...
    }
    {
        ...
        /* if this is a unary op in a declaration, we should indent this token */
        ...
        {
            /* avoid "- --a" becoming "---a" */
            if ((parser_state_tos->last_token == unary_op) && (e_code > s_code) && *res != '!' &&
                (*(e_code - 1) == *res))
            {
                *(e_code++) = ' ';
            }
        ...
        if (parser_state_tos->want_blank &&
            !(parser_state_tos->in_decl && settings.pointer_align_right && *token == '*'))
        {
            set_buf_break(bb_unary_op, paren_target);
            *(e_code++) = ' ';
        }
...
static void
handle_token_binary_op(const bb_code_t can_break)
{
    char *t_ptr;
    if ((parser_state_tos->want_blank || (e_code > s_code && *e_code != ' ')) &&
        !(parser_state_tos->in_parameter_declaration && !settings.pointer_align_right && *token == '*'))
    {
...
static void
handle_token_question(const bb_code_t can_break)
{
    /* This will be used when a later colon appears so
     * we can distinguish the <c>?<n>:<n> construct. */
    squest++;
    if (parser_state_tos->want_blank)
    {
...
static void
handle_token_casestmt(bool *scase, exit_values_t *file_exit_value)
{
...
static void
handle_token_colon(bool *scase, bool *force_nl, int *dec_ind, const bb_code_t can_break, bool *pbreak_line)
{
    char *t_ptr;
    if (squest > 0)
    {
    ...
    else
    {
        ...
        /* This is bullshit for C code, because normally a label doesn't
         * have breakpoints at all of course.  But in the case of wrong code,
         * not clearing the list can make 'indent' core dump. */
        *e_lab++ = ':';
...
static void
handle_token_semicolon(bool *scase, bool *force_nl, bool *sp_sw, int *dec_ind, bool *last_token_ends_sp,
                       exit_values_t *file_exit_value)
{
    ...
    *scase                             = false;
     ...
     *
     * Just because we've found something like extern int foo(); or int (*foo)();
...
static void
handle_token_lbrace(bool *force_nl, int *dec_ind, exit_values_t *file_exit_value, bool *pbreak_line)
{
    parser_state_tos->saw_double_colon = false;
    ...
    if (s_code != e_code && parser_state_tos->block_init != 1)
    {
        if ((!parser_state_tos->in_decl && !settings.btype_2) ||
            (parser_state_tos->in_decl && !settings.braces_on_struct_decl_line && !settings.braces_on_func_def_line))
        {
            dump_line(true, &paren_target, pbreak_line);
            parser_state_tos->want_blank = false;
        }
    ...
    if (s_code == e_code)
    {
        parser_state_tos->ind_stmt = false; /* dont put extra indentation on line with '{' */
    }
    ...
    else
    {
        ...
        /* we cant be in the middle of a declaration,
         * so dont do special indentation of comments */
        parser_state_tos->in_parameter_declaration = 0;
    }
    *dec_ind = 0;
...
static void
handle_token_rbrace(bool *force_nl, int *dec_ind, exit_values_t *file_exit_value, bool *pbreak_line)
{
    char tmpchar[2];
    ...
    parser_state_tos->in_stmt       = false;
    parser_state_tos->block_init_level--;
    if ((parser_state_tos->block_init_level == 0) && (s_code != e_code))
    {
        /* Found closing brace of declaration initialisation,
         * with code on the same line before the brace */
    ...
    else if (parser_state_tos->block_init_level == 1 && settings.spaces_around_initializers)
    {
        /* Put a space before the '}' */
        set_buf_break(bb_rbrace, paren_target);
        *(e_code++) = ' ';
    ...
    *(e_code++)                  = '}';
    ...
    parser_state_tos->search_brace =
        (settings.cuddle_else && (parser_state_tos->p_stack[parser_state_tos->tos] == ifhead)) ||
        (settings.cuddle_do_while && (parser_state_tos->p_stack[parser_state_tos->tos] == dohead));
    ...
    if ((parser_state_tos->p_stack[parser_state_tos->tos] == ifhead) ||
        ((parser_state_tos->p_stack[parser_state_tos->tos] == dohead) && !settings.cuddle_do_while &&
         !settings.btype_2))
    {
        *force_nl = true;
    }
    if (!parser_state_tos->in_decl && (parser_state_tos->tos <= 0) && settings.blanklines_after_procs &&
...
static void
handle_token_swstmt(bool *sp_sw, codes_t *hd_type)
{
    *sp_sw = true;
...
static void
handle_token_sp_paren(bool *sp_sw, codes_t *hd_type)
{
...
static void
handle_token_nparen(bool *force_nl, exit_values_t *file_exit_value, bool *last_else, bool *pbreak_line)
{
    parser_state_tos->in_stmt = false;
    if (*token == 'e')
    {
...
static void
handle_token_overloaded(const bb_code_t can_break)
{
    char *t_ptr;
    if (parser_state_tos->want_blank)
    {
...
static void
handle_token_decl(int *dec_ind, exit_values_t *file_exit_value, bool *pbreak_line)
{
    ...
    else
    {
        ...
        if (parser_state_tos->in_parameter_declaration && (parser_state_tos->dec_nest == 0) &&
            (parser_state_tos->p_l_follow == 0))
        {
            parser_state_tos->ind_level = parser_state_tos->i_l_follow = settings.indent_parameters;
            parser_state_tos->ind_stmt                                 = false;
        }
...
static void
handle_token_ident(bool *force_nl, bool *sp_sw, codes_t *hd_type, int *dec_ind, exit_values_t *file_exit_value,
                   const bb_code_t can_break, bool is_procname_definition, bool *pbreak_line)
{
    ...
    if (parser_state_tos->in_decl && (parser_state_tos->p_l_follow == 0) && (parser_state_tos->last_token != rbrace))
    {
        ...
        parser_state_tos->want_blank = false;
        if ((is_procname_definition == false) || (!settings.procnames_start_line && (s_code != e_code)))
        {
...
static void
handle_token_struct_delim(void)
{
    char *t_ptr;
    for (t_ptr = token; t_ptr < token_end; ++t_ptr)
    {
...
static void
handle_token_comma(bool *force_nl, int *dec_ind, bool is_procname_definition)
{
    parser_state_tos->want_blank = true;

    if ((parser_state_tos->paren_depth == 0) && parser_state_tos->in_decl && !buf_break_used &&
        (is_procname_definition == false) && !parser_state_tos->block_init)
    {
        while ((e_code - s_code) < (*dec_ind - 1))
        {
            check_code_size();
    ...
    *(e_code++) = ',';
    if (parser_state_tos->p_l_follow == 0)
    {
        ...
        /* If we are in a declaration, and either the user wants all comma'd
         * declarations broken, or the line is getting too long, break the line. */
...
/* Preprossesor. */
static void
handle_token_preesc(exit_values_t *file_exit_value, bool *pbreak_line)
{
    char *t_ptr;
    char *p;
    ...
    }
    {
        ...
        if (e_lab - s_lab == com_end && bp_save == 0)
        {
            ...
            if (save_com.end != save_com.ptr)
            {
                ...
                *save_com.end++ = EOL; /* Add newline between comments */
                *save_com.end++ = ' ';
                save_com.len += 2;
                --line_no;
            }
    ...
    else if ((strncmp(p, "else", 4) == 0) || (strncmp(p, "elif", 4) == 0))
    {
        ...
        if (parser_state_tos->next)
        {
            ...
            codes_t        *tos_p_stack       = parser_state_tos->p_stack;
            int             *tos_il            = parser_state_tos->il;
            int             *tos_cstk          = parser_state_tos->cstk;
            short           *tos_paren_indents = parser_state_tos->paren_indents;
            parser_state_t *second            = parser_state_tos->next;
            (void)memcpy(parser_state_tos, second, sizeof(parser_state_t));
            ...
            (void)memcpy(parser_state_tos->p_stack, parser_state_tos->next->p_stack,
                         parser_state_tos->p_stack_size * sizeof(codes_t));
            parser_state_tos->il = tos_il;
            (void)memcpy(
                parser_state_tos->il, parser_state_tos->next->il, (parser_state_tos->p_stack_size * sizeof(int)));
            parser_state_tos->cstk = tos_cstk;
            (void)memcpy(
                parser_state_tos->cstk, parser_state_tos->next->cstk, (parser_state_tos->p_stack_size * sizeof(int)));
    ...
    if ((parser_state_tos->last_token == comma) && (parser_state_tos->p_l_follow <= 0) && settings.leave_comma &&
...
static void
handle_token_comment(bool *force_nl, bool *flushed_nl, bool *pbreak_line)
{
...
extern void
handle_the_token(const codes_t type_code, bool *scase, bool *force_nl, bool *sp_sw, bool *flushed_nl,
                 codes_t *hd_type, int *dec_ind, bool *last_token_ends_sp, exit_values_t *file_exit_value,
                 const bb_code_t can_break, bool *last_else, bool is_procname_definition, bool *pbreak_line)
{
    switch (type_code)
    {
        ...
        case doublecolon :  /* Deal with C++ Class::Method. */
        {
            handle_token_doublecolon();
            break;
        }
        case semicolon :    /* We are not in an initialization or structure declaration. */
        {
         ...
         * Class foo::operator = ()"
         *
         * This is just like a decl, but we need to remember this token type. */
        case overloaded :
        {
            handle_token_overloaded(can_break);
            break;
        }
        case decl : /* We have a declaration type (int, register, etc.) */
        {
            handle_token_decl(dec_ind, file_exit_value, pbreak_line);
            copy_id(type_code, force_nl, file_exit_value, can_break);
            break;
        }
        case cpp_operator : /* Handle C++ operator overloading.  See case overloaded above. */
        case ident :        /* Got an identifier or constant. */
        {
            handle_token_ident(
                force_nl, sp_sw, hd_type, dec_ind, file_exit_value, can_break, is_procname_definition, pbreak_line);
            copy_id(type_code, force_nl, file_exit_value, can_break);
            break;
        }
        case struct_delim :
        ...
        case comment : /* A C or C++ comment. */
        case cplus_comment :
        {
            handle_token_comment(force_nl, flushed_nl, pbreak_line);
            break;
        }
        case attribute : /* An __attribute__ qualifier */
        {

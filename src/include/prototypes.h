...
void extract_segment(linestruct *top, u_long top_x, linestruct *bot, u_long bot_x);
...
char *get_history_completion(linestruct **h, char *s, u_long len);
...
u_long get_statusbar_page_start(u_long base, u_long column);
...
u_long indent_length(const char *line);
u_long quote_length(const char *line);
...
void        recode_NUL_to_LF(char *string, u_long length);
u_long      recode_LF_to_NUL(char *string);
void        free_chararray(char **array, u_long len);
bool        is_separate_word(u_long position, u_long length, const char *buf);
void       *nmalloc(const u_long howmuch);
void       *nrealloc(void *ptr, const u_long howmuch);
char       *measured_copy(const char *string, unsigned long count);
char       *measured_memmove_copy(const char *string, const u_long count);
char       *mallocstrcpy(char *dest, const char *src);
char       *copy_of(const char *string);
char       *memmove_copy_of(const char *string);
char       *free_and_assign(char *dest, char *src);
u_long      get_page_start(unsigned long column);
u_long      xplustabs(void);
u_long      actual_x(const char *text, u_long column);
u_long      wideness(const char *text, u_long maxlen);
u_long      breadth(const char *text);
void        new_magicline(void);
void        remove_magicline(void);
bool        mark_is_before_cursor(void);
void        get_region(linestruct **top, u_long *top_x, linestruct **bot, u_long *bot_x);
void        get_range(linestruct **top, linestruct **bot);
u_long      number_of_characters_in(const linestruct *begin, const linestruct *end);
...
const char *strstr_array(const char *str, const char **substrs, u_int count, u_int *index);
const char *string_strstr_array(const char *str, const vector<string> &substrs, u_int *index);

/* Most functions in 'winio.cpp'. */
void   record_macro(void);
void   run_macro(void);
void   reserve_space_for(u_long newsize);
u_long waiting_keycodes(void);
void   implant(const char *string);
int    get_input(WINDOW *win);
int    get_kbinput(WINDOW *win, bool showcursor);
char  *get_verbatim_kbinput(WINDOW *win, u_long *count);
...
char  *display_string(const char *buf, u_long column, u_long span, bool isdata, bool isprompt);
...
void   draw_row(const int row, const char *converted, linestruct *line, const u_long from_col);
int    update_line(linestruct *line, u_long index, int offset = 0);
int    update_softwrapped_line(linestruct *line);
bool   line_needs_update(const u_long old_column, const u_long new_column);
int    go_back_chunks(int nrows, linestruct **line, u_long *leftedge);
int    go_forward_chunks(int nrows, linestruct **line, u_long *leftedge);
bool   less_than_a_screenful(u_long was_lineno, u_long was_leftedge);
void   edit_scroll(bool direction);
u_long get_softwrap_breakpoint(const char *linedata, u_long leftedge, bool *kickoff, bool *end_of_line);
u_long get_chunk_and_edge(u_long column, linestruct *line, u_long *leftedge);
u_long chunk_for(u_long column, linestruct *line);
u_long leftedge_for(u_long column, linestruct *line);
u_long extra_chunks_in(linestruct *line);
void   ensure_firstcolumn_is_aligned(void);
u_long actual_last_column(u_long leftedge, u_long column);
...
void   spotlight(u_long from_col, u_long to_col);
void   spotlight_softwrapped(u_long from_col, u_long to_col);
...
bool   check_func_syntax(char ***words, u_long *i);
...
char       **words_in_str(const char *str, u_long *size = NULL);
char       **delim_str(const char *str, const char *delim, u_long *size);
...
bool         is_word_func(char *word, u_long *at);
void         remove_leading_char_type(char **word, const char c);
const char  *concat_path(const char *s1, const char *s2);
bool         word_more_than_one_char_away(bool forward, u_long *nchars, const char ch);
bool         word_more_than_one_white_away(bool forward, u_long *nsteps);
bool         word_more_than_one_space_away(bool forward, u_long *nspaces);
bool         word_more_than_one_tab_away(bool forward, u_long *ntabs);
bool         prev_word_is_comment_start(unsigned long *nsteps);
bool         char_is_in_word(const char *word, const char ch, u_long *at);
char        *retrieve_word_from_cursor_pos(bool forward);
char       **fast_words_from_str(const char *str, u_long slen, u_long *nwords);
...
/* 'render.cpp' */
void render_line_text(const int row, const char *str, linestruct *line, const u_long from_col);
void apply_syntax_to_line(const int row, const char *converted, linestruct *line, u_long from_col);
...
void        find_word(linestruct *line, const char *data, const char *word, const u_long slen, const char **start,
                      const char **end);

...
static inline void under_threadpool_mutex(Callback &&callback) {
...
void init_queue_task(void) {
  ...
  task_queue->rear  = 0;
  task_queue->count = 0;
  pthread_mutex_init(&task_queue->mutex, nullptr);
  pthread_cond_init(&task_queue->cond, nullptr);
  ...
  for (Uchar i = 0; i < MAX_THREADS; i++) {
    ...
    if (pthread_create(&threads[i], nullptr, worker_thread, (void *)(uintptr_t)i) != 0) {

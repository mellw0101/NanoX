...
long index_from_mouse_x(const char *string, Uint len, texture_font_t *font, float offset) {
  ...
  for (i=0; i<len; ++i) {
    ...
    prev = cur;
    if (gl_mouse_x() > (double)(st_x + offset) && gl_mouse_x() < (double)(end_x + offset)) {
      break;
    }
  }
...
linestruct *line_from_mouse_y(texture_font_t *font, float offset) {
  ...
  while (line && ++row < openeditor->rows) {
    ...
    if ((gl_mouse_y() > top_y && gl_mouse_y() < bot_y) || (gl_mouse_y() < top_y && gl_mouse_y() < bot_y)) {
      break;
    }
    line = line->next;
  }
...
linestruct *line_and_index_from_mousepos(texture_font_t *const font, Ulong *const index) {
  ...
  *index = index_from_pix_xpos(line->data, gl_mouse_x(), openeditor->text->x, font);
  return line;
}

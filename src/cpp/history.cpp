...
char *
get_history_completion(linestruct **here, char *string, unsigned long len)
{
    linestruct *htop = nullptr, *hbot = nullptr;
    linestruct *item;
    ...
    while (item != nullptr && strcmp(item->data, string) == 0)
    {
...
load_poshistory(void)
{
    ...
    while ((read = getline(&stanza, &dummy, histfile)) > 5)
    {
        ...
        *(lineptr++)   = '\0';
        ...
        newitem               = (poshiststruct *)nmalloc(sizeof(poshiststruct));
...
void
reload_positions_if_needed(void)
{
...
update_poshistory()
{
    ...
    if (fullpath == nullptr || openfile->filename[0] == '\0')
    {
        free(fullpath);
        return;
    }
